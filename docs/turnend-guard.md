...
Harness-specific tracked hook files only adapt each integrated harness's real turn-end mechanism to that shared predicate.
...
Claude, Codex, OpenCode, and Grok have tracked primary integrations:
...
OpenCode and Grok expose passive lifecycle callbacks for this purpose.
...
These integrations were validated on 2026-07-08 in scratch repos or throwaway homes, not against the captain's live primary fleet state.
...
In noninteractive `opencode run`, `promptAsync` returned successfully but the process exited before displaying the follow-up, so this adapter is trusted for persistent firstmate TUI sessions and documented as passive/fail-open in headless mode.
...
If Grok declines to load project hooks, this guard fails open and `fm-guard.sh` remains the next-command alarm.
...
`tests/fm-turnend-guard.test.sh` covers the shared predicate, primary scoping (including a secondmate's own home being guarded like the main primary while its child worktrees stay exempt), `FM_HOME` and `FM_STATE_OVERRIDE` precedence, fail-open behavior without `jq`, tracked hook registration for the four integrated harnesses, and the Grok adapter's forced-resume loop guard and permission-mode regression.
