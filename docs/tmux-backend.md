...
- `alive` - the name contains `claude`, `codex`, `opencode`, or `grok`, or a node-reported pane's exact foreground process matches the verified Pi CLI shape below. The four direct-name harnesses were confirmed to run as their own literal process name (`ps -ef`, 2026-07-07): `claude` and `codex` and `opencode` are each a native compiled binary (`file` reports Mach-O), so their `comm` is their own binary name with no interpreter wrapper to hide behind.
...
### Verified Pi CLI process probe

Pi is installed as a Node CLI, and tmux 3.6a reports a live Pi 0.80.6 pane as `pane_current_command=node` even though the foreground process has the command/argv identity `pi pi` on the verified Bun installation.
A generic `node` value alone remains ambiguous and is never classified alive or dead.

Verification on 2026-07-10 used a dedicated tmux socket and a real interactive Pi 0.80.6 process.
The exact observations were:

```text
pane_current_command=node
pane_pid=51880
foreground_pgid=52106
51880 51879 51880 zsh zsh
52106 51880 52106 pi  pi
```

`fm_backend_tmux_pi_process_alive` now resolves that exact foreground process without guessing from `node`.
It reads `#{pane_pid}`, queries `ps -o tpgid=` for the pane tty's foreground process-group leader, then reads `comm` and `args` for that PID.
It returns success only when command basename and argv0 both identify `pi`, or when Node's arguments contain the installed `@earendil-works/pi-coding-agent/dist/cli.js` entrypoint.
A generic `node server.js`, malformed process-group ID, missing process, or unreadable value remains `unknown`.
A pane that has returned to a bare shell remains `dead`.

Command run for the mandatory classifier regression: `tests/fm-secondmate-liveness.test.sh`.
Observed output includes `ok - fm_backend_tmux_agent_alive: alive/dead/unknown classification` and confirms verified Pi, generic Node, malformed process data, and exited-shell cases.
...
Pi liveness depends on the foreground process retaining one of the verified CLI identities; any other Node process remains safely inconclusive.
