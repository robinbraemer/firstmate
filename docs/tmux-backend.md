...
The observed pane command was `node`, while the foreground process command and argv both identified `pi`.
...
The classifier regression is `tests/fm-secondmate-liveness.test.sh`; it covers verified Pi, generic Node, malformed process data, and exited-shell cases.
