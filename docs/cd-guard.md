...
| Pi | `.pi/extensions/fm-primary-pi-watch.ts` `tool_call` handler | Returns `{block: true}`; piggybacks on the already-loaded watcher extension so no extra `-e` flag is needed. |
...
The Pi command uses the current watcher-extension carrier. The 2026-07-11 validation predates the watcher-only lifecycle change and is historical evidence only; it is not current wiring.
