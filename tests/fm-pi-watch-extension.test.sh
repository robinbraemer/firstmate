...
test_tracked_extension_present_and_self_hashing() {
  ...
  assert_not_contains "$text" "sendUserMessage" "tracked extension still manufactures user-role wake messages"
...
test_pi_live_acceptance_helper_records_isolated_evidence() {
  local helper home pi_dir evidence fakebin candidate out status hash_count session_pid watcher_pid
...
SH
  ...
  assert_contains "$(cat "$evidence/armed-watcher-lock.txt")" "watcher_pid=$watcher_pid" "acceptance snapshot omitted watcher ownership"
