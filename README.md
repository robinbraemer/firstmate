...
- **Event-driven, zero-token supervision** - a bash watcher sleeps on the fleet and wakes the first mate only when something needs you; verified non-Pi primary harnesses also get a turn-end backstop that blocks or follows up on a blind stop when work is in flight and supervision is not live, while Pi uses watcher-only native wake delivery.
...
- [docs/turnend-guard.md](docs/turnend-guard.md) - the primary session's structural "no turn ends blind" backstop for verified non-Pi harnesses: hook mechanisms, scoping, loop safety, and fail-open tradeoffs.
